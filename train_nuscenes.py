...
def compute_frame_to_frame_transform_from_batch(egopose):
    """
    从训练批次中的 egopose 计算帧间的 3D 转换矩阵。
    
    Args:
        egopose: Tensor (B, T, 4, 4)，包含每个批次、时间帧的 egopose。
        
    Returns:
        T_frame2_to_frame1: Tensor (B, T-1, 4, 4)，帧间的 3D 转换矩阵。
    """
    B, T, _, _ = egopose.shape
    T_frame2_to_frame1 = torch.zeros((B, T - 1, 4, 4), device=egopose.device)

    for t in range(T - 1):
        T_ego_t = egopose[:, t, :, :]
        T_ego_t_curr = egopose[:, -1, :, :]
        
        # 计算从帧 t 到帧 t+1 的变换T_ego_t_plus_1
        T_frame2_to_frame1[:, t, :, :] = torch.matmul(T_ego_t_curr, torch.linalg.inv(T_ego_t))

    return T_frame2_to_frame1

def extract_bev_transform_from_3d(T_frame2_to_frame1):
    """
    从 3D 转换矩阵提取 BEV 转换。
    ...
    Returns:
        T_bev: Tensor (B, T-1, 3, 3)，帧间的 BEV 转换矩阵。
    """
    B, T_minus_1, _, _ = T_frame2_to_frame1.shape
    T_bev = torch.eye(3, device=T_frame2_to_frame1.device).unsqueeze(0).repeat(B, T_minus_1, 1, 1)
    
    for b in range(B):
        for t in range(T_minus_1):
            # 提取平面旋转和平移
            rotation = T_frame2_to_frame1[b, t, :2, :2]
            translation = T_frame2_to_frame1[b, t, :2, 3]
            
            T_bev[b, t, :2, :2] = rotation
            T_bev[b, t, :2, 2] = translation
...
def run_model(model, loss_fn, d, device='cuda:0', sw=None):
    ...
    (imgs, rots, trans, intrins, pts0, extra0, pts, extra, lrtlist_velo, vislist, 
    tidlist, scorelist, seg_bev_g, valid_bev_g, center_bev_g, offset_bev_g, radar_data, egopose)= d

    B0,T,S,C,H,W = imgs.shape
    assert(T>=1)
    
    # 获取帧间 3D 转换矩阵
    T_frame2_to_frame1 = compute_frame_to_frame_transform_from_batch(egopose)

    # 提取帧间 BEV 转换矩阵
    T_bev = extract_bev_transform_from_3d(T_frame2_to_frame1)
    ...
    pts_all=pts
    extra_all=extra
    lrtlist_velo_all=lrtlist_velo
    vislist_all=vislist
    tidlist_all=tidlist
    scorelist_all=scorelist
    seg_bev_g_all=seg_bev_g
    valid_bev_g_all=valid_bev_g
    center_bev_g_all=center_bev_g
    offset_bev_g_all=offset_bev_g
    radar_data_all=radar_data
    for t in range(T):
    # eliminate the time dimension
        ...
        egopose_t = egopose_all[:,t]
        
        #下面一部分主要和点云以及训练集本身参数有关
        pts0 = pts0_all[:,t]
        extra0 = extra0_all[:,t]
        pts = pts_all[:, t]
        extra = extra_all[:, t]
        ...
        origin_T_velo0t = egopose_t.to(device) # B,T,4,4
        ...
        rgb_list.append(rgb_camXs)            # (B, S, C, H, W) or (B, C, H, W)
        pix_T_cams_list.append(pix_T_cams)    # (B, 4, 4)
        egopose_list.append(egopose_t)
        cam0_T_camXs_list.append(cam0_T_camXs)
        
    
    '''
    将其堆叠T维度
    '''
    rgb_camXs = torch.stack(rgb_list, dim=1)  # dim=1 表示 T
    pix_T_cams = torch.stack(pix_T_cams_list, dim=1)  
    # rad_occ_mem0 = torch.stack(rad_occ_mem0_list, dim=1)
    #aleady have T_bev
    cam0_T_camXs = torch.stack(cam0_T_camXs_list, dim=1)
...
def main(
    ...
    train_dataloader, val_dataloader = nuscenesdataset.compile_data(
        ...
        seqlen=seqlen, # we do not load a temporal sequence here, but that can work with this dataloader
        nsweeps=nsweeps,
